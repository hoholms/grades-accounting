...
public class Window extends JFrame {
    ...
    private void setComponents() {
        ...
        plus.addActionListener(e -> {
            ...
            if (result != null && result.length() > 0) {
                ...
                subjPanels.add(addSubjPanel(subjects.lastElement(), subjPanels));
                subjPanels.updateUI();
            }
        });
    ...
    private JTextField addGradeTf(Subject subj, int ind, JLabel subjAvg) {
        ...
        if (subj.getGrades().elementAt(ind) != null)
            tf.setText("" + subj.getGrades().elementAt(ind));
    ...
    private float globalGrade(Vector<Subject> subjects) {
        float avg = 0.0f;
        for (Subject sbj : subjects) {
            if (!sbj.isExam())
                avg += sbj.avgGrade();
